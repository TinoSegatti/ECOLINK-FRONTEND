...
export default function useClientes() {
  ...
  const [precioForm, setPrecioForm] = useState<{ tipoCliente: string; nuevoPrecio: number | null }>({
    tipoCliente: "",
    nuevoPrecio: null,
  })
  ...
  const handleActualizarPrecios = async () => {
    if (!precioForm.tipoCliente || precioForm.nuevoPrecio === null) {
      return { success: false, errors: [{ field: "general", message: "Tipo de cliente y precio son requeridos" }] }
    }
    try {
      const response = await actualizarPrecios(precioForm.tipoCliente, precioForm.nuevoPrecio)
      if (response.success && response.data) {
        // Refrescar lista de clientes para reflejar los nuevos precios
        const updatedClientes = await fetchClientes()
        setClientes(updatedClientes)
        setPrecioForm({ tipoCliente: "", nuevoPrecio: null })
      ...
      return { success: false, errors: [{ field: "general", message: "Error al actualizar precios" }] }
    }
  }
