...
const ActualizarPreciosResponseSchema = z.object({
  message: z.string(),
  count: z.number(),
})
...
export const actualizarPrecios = async (tipoCliente: string, nuevoPrecio: number): Promise<ApiResponse<{ message: string; count: number }>> => {
  return fetchWithErrorHandling(
    `${API_URL}/clientes/actualizar-precios`,
    {
      method: "POST",
      body: JSON.stringify({ tipoCliente, nuevoPrecio }),
    },
    ActualizarPreciosResponseSchema,
